  ...
  <div id="feedback_all">
    <el-container>
      <el-header class="page-header">
        <h1><i class="fa fa-table"></i>用户反馈</h1>
        <el-breadcrumb separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item><i class="fa fa-question-circle"></i>用户反馈</el-breadcrumb-item>
        </el-breadcrumb>
      </el-header>
      <el-container>
        <el-main>
          ...
          <el-table class="goodsTable" :data="tableData" v-loading="!tableLoading" @selection-change="handleSelect"
            ...
            <el-table-column sortable prop="phone" label="电话"></el-table-column>
            <el-table-column sortable prop="type_" label="反馈类型"></el-table-column>
          ...
          <el-row class="table-operate">
            <el-button v-if="myData.replyStatus == 1||myData.replyStatus==undefined" type="success" plain @click="reply(0)"
                       :disabled="disabled">批量回复
            </el-button>
            <el-button v-if="myData.replyStatus !=0" type="info" plain @click="ban(0)" :disabled="disabled">批量关闭</el-button>
          </el-row>
  ...
  export default {
    ...
    methods:{
      ...
      async reply(flag) {
        ...
        var res = await tool.showModal(that, '确定已回复?', true, '是的', '提示', 'warning')
        ...
        res = await tool.post(myData.module + '/reply', postData, that)
